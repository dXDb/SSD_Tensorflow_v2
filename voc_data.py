...
from image_utils import random_patching, random_brightness, random_contrast, random_hue, random_saturation, horizontal_flip
...
class VOCDataset():
    ...
    def __init__(self, root_dir, year, default_boxes,
        ...
        ## WJ ##
        self.idx_to_name = ['bus', 'car', 'cat', 'cow', 'horse']
        with open('./data_txt/train/dataset.txt') as f: self.train_ids = f.readlines()
        with open('./data_txt/val/dataset.txt') as f: self.val_ids = f.readlines()
        self.subset = 'train'
        ...
        self.train_ids = copy.deepcopy(temp_list)

        temp_list = []
        for i in self.val_ids:
            temp_list.append(i.split("\n")[0])
        self.val_ids = copy.deepcopy(temp_list)
        ...
        if num_examples != -1:
            self.ids = self.train_ids[:num_examples]
    ...
    def _get_image(self, index):
        ...
        ## WJ ##
        if self.subset == 'train':
            filename = self.train_ids[index]
        elif self.subset == 'val':
            filename = self.val_ids[index]
    ...
    def _get_annotation(self, index, orig_shape):
        ...
        for obj in objects:
            ...
            ## WJ ##
            if not name in self.idx_to_name: continue
    ...
    def generate(self, subset=None):
        ...
        for index in range(len(indices)):
            ...
            if augmentation_method == 'patch':
                ...
                img, boxes, labels = random_brightness(img, boxes, labels)
            elif augmentation_method == 'contrast':
                img, boxes, labels = random_contrast(img, boxes, labels)
            elif augmentation_method == 'hue':
                img, boxes, labels = random_hue(img, boxes, labels)
            elif augmentation_method == 'saturation':
                img, boxes, labels = random_saturation(img, boxes, labels)
...
def create_batch_generator(root_dir, year, default_boxes,
    ...
    if mode == 'train':
        ...
        test_gen = partial(voc.generate, subset='val')
